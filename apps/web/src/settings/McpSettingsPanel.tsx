...
export function McpSettingsPanel({ value, onChanged, onMessage }: Props) {
  ...
  return <section className="mcp-settings">
    <div className="mcp-adapter-line"><span><i />{value?.adapter.package ?? "pi-mcp-adapter"} <b>v{value?.adapter.version ?? "—"}</b></span><button className="secondary" onClick={() => { setEditingName(undefined); setDraft(blankDraft()); }}>添加 MCP 服务器</button></div>
    ...
    {!value?.servers.length ? <div className="skills-empty">尚未配置 MCP 服务器。</div> : null}
