...
export function LiteratureWorkbenchApp({ onNavigate }: { onNavigate?: (app: "research") => void } = {}) {
  ...
  useEffect(() => {
    setEvidence(listEvidence(PROJECT_ID));
    const records = listEvidence(PROJECT_ID);
    ...
    // 本地版暂无 Research Graph 投影：主张列表保持为空，选择器仍可"暂不关联主张"
    setClaimRevisions([]);
  }, []);
  ...
  if (!graph) return <div className="literature-workbench-root"><div className="literature-start">
  ...
  return <div className="literature-workbench-root"><div className={`paper-graph-view ${detailOpen ? "detail-open" : "detail-closed"}`}>
