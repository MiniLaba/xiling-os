...
自动门禁把动态窗口生产包限制在 350 KB 以内，并在真实 Electron 启动中使用固定的 1040×700 验收视口，实际点击程序坞、确认工作台与缩放入口出现、唤醒 Core，稳定一秒后检查总活动工作集不超过 520 MB。当前实测约 495 MB。该开发/CI 回归线上限来自 Electron 44 在 macOS 上包含 Browser、GPU、Network Service、Renderer 和 Core Utility Process 的真实五进程基线；它不替代发布设备上 250 MB 的空闲目标，D2 结束前还需按进程优化并收紧。
