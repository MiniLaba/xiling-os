...
- Docker Runner、容器镜像、容器健康检查和容器发布门禁
- 开发期数据库和示例数据
- 科研图谱中缺乏科学语义的节点与关系
...
Desktop V2 不依赖 Docker。第一阶段仅开放受信任的内置科研配方，并使用锁定环境、输入快照、资源预算、取消令牌和输出哈希。模型生成代码、第三方插件代码和不可信脚本保持禁用，直到 macOS、Windows、Linux 的系统级沙箱适配器分别通过逃逸、文件越权、网络和资源耗尽测试。不得用普通子进程冒充沙箱。
