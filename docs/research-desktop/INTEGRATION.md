...
- [ ] 抽取并连接统一科研应用服务，移除桌面证据 localStorage 写路径。
- [ ] 统一 Task/Session/Artifact/Approval；接入 Pi 科研 Runtime。
- [ ] 科研页面窗口化，逐窗口 ProjectScope，统一设计组件与主题。
- [ ] 双语音/伴侣切换为科研服务入口，保留音频原生语义。
- [ ] 无 Docker 安装启动；执行适配器安全能力真实声明。
- [ ] 小型真实科研闭环和窗口交互验收。
- [ ] 新整合 PR；明确替代旧 PR #17，但不自动合并。

## 验收

2026-09-10 本轮证据：26 个工作区恢复，全部构建通过；桌面 typecheck 通过。共享图谱/服务投影 10 项测试通过，桌面知识持久化及 OS 内核 73 项通过。文献 localStorage 写路径已移除，项目作用域和重启恢复有测试；原 localStorage 数据未删除。共享 tokens 已从同一源生成；最终 UI 真机检查未完成。

待完成的关键项仍是 Pi/Task/Artifact/Approval 主路径归一、项目/Wiki/科研画布窗口化、单一科研数据根和真实闭环。这些不能因本轮构建成功勾选。当前仓库属于整合中的开发基线，不是完成发布。
