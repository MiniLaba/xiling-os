...
| 应用内多窗口 | 基础已实现 | 一个 `BrowserWindow`；React Window Manager 按需载入、拖动、聚焦、最大化、最小化卸载与布局保存 |
...
| 对话、研究、文献、数据、设置 | 窗口入口已实现 | 当前为空状态/能力占位，不宣称旧版功能已迁移 |
...
当前静态工作台/关于窗口是 V2 过渡壳。新增应用必须进入 React Window Manager，不再扩展静态 DOM 窗口。
...
- 动态 React 窗口生产包：自动回归上限 350 KB；当前 gzip 约 72 KB。
- 真实 Electron 打开内部窗口并唤醒 Core后的活动工作集：开发/CI 回归上限 450 MB。
- 文件列表、日志、文献、事件和图节点必须虚拟化/有界查询。
- Core、Agent、索引器、Python、MCP 和领域运行时必须接入统一生命周期，禁止登录即全开。

250 MB 是空闲产品目标，450 MB 是当前活动场景的防退化上限，两者不得混用。
...
- 动态窗口包未进入冷启动、包体上限；
- 真实 Electron 中实际点击 Dock、打开 React 内部窗口、唤醒 Core 与活动内存上限。

## 14. 后续顺序

1. 把静态过渡工作台迁入 React Window Manager，完成文档型多实例、尺寸调整、窗口菜单、键盘导航和崩溃恢复。
