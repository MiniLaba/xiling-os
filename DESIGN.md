...
| 真实桌面文件夹 | D2 基础已实现 | 原生目录选择、`workspace://`、目录导航、列表/搜索、新建、重命名、剪切移动、系统废纸篓、限量文本预览、桌面图标、拖入和本机打开；富媒体预览与虚拟化待实现 |
...
- 文件夹导航和剪切移动只传递 `workspace://`；移动目标必须是同一根目录内的真实文件夹，禁止把目录移入自身。文本预览最多读取 512 KB，界面默认请求 256 KB；不支持格式只显示元数据并交给系统打开。
...
- 真实 Electron 打开工作台内部窗口并唤醒 Core 后的活动工作集：开发/CI 回归上限 520 MB；本轮实测约 461 MB。
...
- Unicode/空格文件、原子导入、搜索、新建、重命名、同根移动、自身嵌套拒绝、限量文本预览、二进制降级、重名/非法名/越界与符号链接保护；
- 原生目录变化推送和监听器关闭；
...
2. 在已完成目录导航、搜索、新建、重命名、剪切移动、系统废纸篓和文本预览的基础上，补齐图像/PDF/科研格式预览与大目录虚拟化。
